...
namespace ActivityManagement.EntityFrameworkCore.Seed
{
    public class SeedDataBuilder
    {
        ...
        public void Create()
        {
            SeedDefaultTenant();
            SeedEmployees();
            SeedTeam();               // Infrastructure Team — 6 kişi üye, lider Emin Akkurt
            ...
            // SeedActivities();  // devre dışı — dummy faaliyet (ActivityLog) kayıtları
            _context.SaveChanges();
        }

        // Tüm personeli tek "Infrastructure Team" takımına bağlar; lideri Emin Akkurt.
        // Idempotent: mevcut "Infrastructure"/"Infrastructure Team" satırını onarır (ad + lider),
        // takımı olmayan çalışanları bu takıma atar (başka takıma taşınmış olanlara dokunmaz).
        private void SeedTeam()
        {
            var leader = _context.Employees
                .FirstOrDefault(e => e.FirstName == "Emin" && e.LastName == "Akkurt");

            var team = _context.Teams
                .FirstOrDefault(t => t.Name == "Infrastructure Team" || t.Name == "Infrastructure");
            if (team == null)
            {
                team = new Team
                {
                    ...
                    Description = "Altyapı ekibi (sistem, network, güvenlik, bulut).",
                    IsActive = true
                };
        ...
        // Rol tanımları (sistem rolleri) + Rol×Sayfa erişim matrisi. Idempotent: eksikleri tamamlar.
        private void SeedRolesAndAccess()
        {
            // Sistem rolleri
            foreach (var (name, display) in ActivityManagement.Authorization.PageCatalog.SystemRoles)
            {
                if (!_context.AppRoles.Any(r => r.Name == name))
                    _context.AppRoles.Add(new AppRoleDef { Name = name, DisplayName = display, IsSystem = true, SortOrder = 0 });
            }
            _context.SaveChanges();

            // Varsayılan erişim: yalnızca hiç kaydı olmayan (rol,sayfa) için eklenir (mevcut ayarlar korunur)
            var existing = _context.RolePageAccesses.Select(x => x.RoleName + "|" + x.PageKey).ToHashSet();
            foreach (var kv in ActivityManagement.Authorization.PageCatalog.DefaultAccess)
            {
                var role = kv.Key;
                foreach (var page in ActivityManagement.Authorization.PageCatalog.Pages)
                {
                    var key = role + "|" + page.Key;
        ...
        {
            if (_context.ActivityTypes.Any()) return;
            var names = new[] { "Destek", "Bakım", "Geliştirme", "Kurulum", "Toplantı", "İnceleme", "Test", "Dokümantasyon", "Eğitim", "Analiz", "Operasyon", "Diğer" };
            int order = 1;
            foreach (var n in names)
                _context.ActivityTypes.Add(new ActivityTypeDef { TenantId = 1, Name = n, SortOrder = order++, IsActive = true });
            _context.SaveChanges();
        }

        // Pano/Kanban kolonlarının kaynağı. Boşsa varsayılan 5 durum eklenir.
        // StatusValue, TaskItem.Status (TaskStatus enum) tamsayı değeriyle eşleşmelidir; yoksa
        // görevler hiçbir kolona düşmez ve Pano boş görünür.
        private void SeedWorkflowStatuses()
        {
            if (_context.WorkflowStatuses.Any()) return;

            _context.WorkflowStatuses.AddRange(
                new WorkflowStatus { Name = "Beklemede",    Color = "secondary", SortOrder = 1, IsActive = true, IsCompletedState = false, StatusValue = (int)Entities.TaskStatus.Beklemede },
                new WorkflowStatus { Name = "Devam Ediyor", Color = "primary",   SortOrder = 2, IsActive = true, IsCompletedState = false, StatusValue = (int)Entities.TaskStatus.DevamEdiyor },
                new WorkflowStatus { Name = "Tamamlandı",   Color = "success",   SortOrder = 3, IsActive = true, IsCompletedState = true,  StatusValue = (int)Entities.TaskStatus.Tamamlandi },
                new WorkflowStatus { Name = "İptal",        Color = "dark",      SortOrder = 4, IsActive = true, IsCompletedState = false, StatusValue = (int)Entities.TaskStatus.Iptal },
                new WorkflowStatus { Name = "Ertelendi",    Color = "warning",   SortOrder = 5, IsActive = true, IsCompletedState = false, StatusValue = (int)Entities.TaskStatus.Ertelendi }
            );
            _context.SaveChanges();
        }
